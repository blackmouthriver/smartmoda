...
| **R-33** | Alto | La defensa completa contra fuerza bruta no es aplicable en el plan gratuito de Supabase | Sin bloqueo por cuenta ni respuestas indistinguibles hasta el Sprint 6. Una cuenta concreta puede ser atacada con más margen del previsto | CAPTCHA en Attack Protection y límites de tasa de Supabase, que sí corren en el endpoint real. El código de `BruteForcePolicy` y del hook queda escrito y probado, listo para activarse al subir de plan o al llegar `core-api` (`EN-1201`) | Seguridad |
